...
+ (void)showToastWithString:(NSString *)text withView:(UIView *)view {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        hud.label.text = @"";
        hud.detailsLabel.font = [UIFont fontWithName:@"HelveticaNeue" size:15.0];
...
+ (NSString *)getPhoneForCall:(NSString *)phone {
    ...
    if (phone.length) {
        strResult = [[phone stringByReplacingOccurrencesOfString:@"+" withString:@""] stringByReplacingOccurrencesOfString:@" " withString:@""];
        ...
        } else if ([firstChar isEqualToString:@"1"]) {
            strResult = [prefixPhone stringByAppendingString:strResult];
        }
    }
...
BOOL STEValidateEmail(NSString *checkString) {
