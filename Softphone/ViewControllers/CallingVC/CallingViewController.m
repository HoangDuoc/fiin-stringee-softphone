    ...
    if (!hasCreatedCall) {
        ...
        if (self.isIncomingCall) {
            ...
            [self.stringeeCall makeCallWithCompletionHandler:^(BOOL status, int code, NSString *message, NSString *data) {
                NSLog(@"makeCallWithCompletionHandler %@ -- %@", message, data);
                if ([data isEqualToString:@"NOT_ENOUGH_MONEY"]) {
                    isNotEnoughMoney = YES;
                }
                if (!status) {
...
- (void)endCallAndDismissWithTitle:(NSString *)title {
    ...
    [Utils delayCallback:^{
        ...
        [vc dismissViewControllerAnimated:YES completion:^{
            [CallManager sharedInstance].currentCall = nil;
            [SPManager instance].callingViewController = nil;
            if (haveNoNumberToCallOut) {
                [Utils showToastWithString:@"Không có số để gọi ra. Kiểm tra trong Cài Đặt" withView:nil];
            }
            if (isNotEnoughMoney) {
                [Utils showToastWithString:@"Tài khoản không đủ để thực hiện cuộc gọi" withView:nil];
            }
        
            [[SPManager instance] checkBalanceForPhone:[SPManager instance].myUser.phone];
