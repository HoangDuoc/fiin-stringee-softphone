    ...
    if (@available(iOS 9, *)) {
        ...
        [store requestAccessForEntityType:CNEntityTypeContacts completionHandler:^(BOOL granted, NSError * _Nullable error) {
            ...
            for (CNContact *contact in contacts) {
                ...
                if ([formatter stringFromContact:contact].length) {
                ...
                if (values.count) {
                    for (int i = 0 ; i < values.count ; i++) {
                        phoneNumber = (NSString *)[(contact.phoneNumbers[i].value) valueForKey:@"digits"];

                        [phones setObject:username forKey:[Utils getPhoneForCall:phoneNumber]];
        ...
        NSMutableDictionary *phonesToCheck = [[NSMutableDictionary alloc] init];
        NSMutableArray *privateContacts = [[NSMutableArray alloc] init];
        ...
        for(int i = 0; i < numberOfPeople; i++){
            ...
            for(CFIndex i = 0; i < ABMultiValueGetCount(phones); i++) {
                ...
                [phonesToCheck setObject:username forKey:[Utils getPhoneForCall:phoneNumber]];
                ContactModel *privateContact = [[ContactModel alloc] initWithName:username phone:phoneNumber];
                [privateContacts addObject:privateContact];
            }
        }
...
- (void)checkPhonesExistedWithPrivateContacts:(NSMutableArray *)privateContacts phones:(NSMutableDictionary *)phones {
    
    NSString *token;
    if ([SPManager instance].isAppleCheck) {
        token = @"";
    } else {
        token = [SPManager instance].myUser.token;
    }
    
    // reload table luôn không sẽ phải đợi nếu có nhiều contact
    dispatch_async(dispatch_get_main_queue(), ^{
...
//    [GlobalService checkPhoneBookExistedWithPhoneBook:(NSDictionary *)phones token:token completionHandler:^(BOOL status, int code, id responseObject) {
//        dispatch_async(dispatch_get_main_queue(), ^{
//            if (status) {
//                // Thành công
...
//                    for (ContactModel *privateContact in privateContacts) {
//                        NSLog(@"phone_call %@ - phone %@", privateContact.phone_call, phone);
