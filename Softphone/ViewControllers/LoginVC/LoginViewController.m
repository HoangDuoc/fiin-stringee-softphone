...
- (IBAction)loginTapped:(UIButton *)sender {
    ...
    if ([self.tfPhoneNumber.text isEqualToString:@"0123456789"]) {
        // Xử lý cho số đặc biệt để cho thằng apple nó test
        [GlobalService loginWithPhoneNumber:[Utils getPhoneForCall:self.tfPhoneNumber.text] completionHandler:^(BOOL status, int code, id responseObject) {
            ...
            if (status) {
                ...
                NSLog(@"Apple Token %@", token);
                [SPManager instance].isAppleCheck = YES;
                [SPManager instance].isAppleToken = token;
